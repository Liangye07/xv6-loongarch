//
// virtio-blk driver over PCI (modern virtio-pci).
// LoongArch QEMU "virt" machine exposes block devices via PCI.
//
...
#define VIRTIO_PCI_DEVICE_BLK      0x1001  // legacy transitional virtio-blk
#define VIRTIO_PCI_DEVICE_BLK_MOD  0x1042  // modern virtio-blk

// QEMU loongarch virt: PCI ECAM/MMCONFIG base (physical).
// If your QEMU machine differs, adjust this constant.
...
static inline void pci_write32(int bus, int dev, int fn, int off, uint32 v)
{
  *(volatile uint32 *)(ecam_fn_base(bus, dev, fn) + off) = v;
}
...
static inline uint64 pci_read_bar_addr(int bus, int dev, int fn, int bar_idx)
{
  ...
  if((lo & 0x1) != 0)
    return 0; // IO BAR unsupported here

  if((lo & 0x6) == 0x4){ // 64-bit mem BAR
    uint32 hi = pci_read32(bus, dev, fn, off + 4);
    uint64 addr = ((uint64)hi << 32) | (lo & ~0xFULL);
    return addr;
  }
...
static int
virtio_pci_find_blk(int *out_bus, int *out_dev, int *out_fn)
{
  for(int dev = 0; dev < 32; dev++){
    int fnmax = 1;
    uint8 ht = pci_read8(0, dev, 0, PCI_HEADER_TYPE_OFF);
    if(ht & 0x80)
      fnmax = 8;
...
virtio_disk_init(void)
{
  ...
  int bus, dev, fn;
  if(virtio_pci_find_blk(&bus, &dev, &fn) < 0)
    panic("virtio-pci blk not found");

  uint16 cmd = pci_read16(bus, dev, fn, PCI_COMMAND_OFF);
  cmd |= (PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER);
  pci_write16(bus, dev, fn, PCI_COMMAND_OFF, cmd);

  volatile uint8 *fncfg = ecam_fn_base(bus, dev, fn);
  ...
  while(cap){
    ...
    if(cap_id == PCI_CAP_ID_VENDOR){
      ...
      c.cap_vndr = pci_read8(bus, dev, fn, cap + 0);
      c.cap_next = pci_read8(bus, dev, fn, cap + 1);
      c.cap_len = pci_read8(bus, dev, fn, cap + 2);
      c.cfg_type = pci_read8(bus, dev, fn, cap + 3);
      c.bar = pci_read8(bus, dev, fn, cap + 4);
      c.id = pci_read8(bus, dev, fn, cap + 5);
      c.padding[0] = pci_read8(bus, dev, fn, cap + 6);
      c.padding[1] = pci_read8(bus, dev, fn, cap + 7);
      c.offset = pci_read32(bus, dev, fn, cap + 8);
      c.length = pci_read32(bus, dev, fn, cap + 12);

      uint64 bar_pa = pci_read_bar_addr(bus, dev, fn, c.bar);
      if(bar_pa != 0){
        volatile uint8 *base = (volatile uint8 *)(bar_pa | DMWIN1_MASK);
        if(c.cfg_type == VIRTIO_PCI_CAP_COMMON_CFG){
          disk.common = (volatile struct virtio_pci_common_cfg *)(base + c.offset);
        } else if(c.cfg_type == VIRTIO_PCI_CAP_NOTIFY_CFG){
          disk.notify_base = base + c.offset;
          disk.notify_off_multiplier = pci_read32(bus, dev, fn, cap + 16);
        } else if(c.cfg_type == VIRTIO_PCI_CAP_ISR_CFG){
          disk.isr = base + c.offset;
        }
      }
    }
  ...
  if(disk.common == 0 || disk.notify_base == 0 || disk.isr == 0)
  ...
  // feature negotiation: keep minimal feature set
  disk.common->device_feature_select = 0;
  uint32 features = disk.common->device_feature;
  ...
  disk.common->driver_feature = features;
...
static void
virtio_disk_complete_used(void)
{
...
virtio_disk_rw(struct buf *b, int write)
{
  ...
  *notify = 0;
  ...
  // polling completion keeps driver working even before IRQ routing is set up.
  while(b->disk == 1)
...
{
  acquire(&disk.vdisk_lock);
  (void)(*disk.isr); // read to acknowledge ISR for modern virtio-pci
  virtio_disk_complete_used();
  release(&disk.vdisk_lock);
}
