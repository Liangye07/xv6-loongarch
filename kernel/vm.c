...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 3; level > 0; level--) {
    ...
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)(PTE2PA(*pte) | DMWIN0_MASK);      
    } else {
...
freewalk(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V) && (PTE_FLAGS(pte) == PTE_V)){
      ...
      uint64 child = (PTE2PA(pte) | DMWIN0_MASK);
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((mem = kalloc()) == 0)
      goto err;
    memmove(mem, (char*)(pa | DMWIN0_MASK), PGSIZE);
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    if(n > len)
      n = len;
    memmove((void *)((pa0 + (dstva - va0)) | DMWIN0_MASK), src, n);
...
copyin(pagetable_t pagetable, char *dst, uint64 srcva, uint64 len)
{
  ...
  while(len > 0){
    ...
    if(n > len)
      n = len;
    memmove(dst, (void *)((pa0 + (srcva - va0)) | DMWIN0_MASK), n);
...
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  ...
  while(got_null == 0 && max > 0){
    ...
    char *p = (char *) ((pa0 + (srcva - va0)) | DMWIN0_MASK);
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
        got_null = 1;
