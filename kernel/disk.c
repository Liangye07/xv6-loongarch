...
virtio_pci_parse_caps(uint bus, uint dev, uint fun)
{
  ...
  printf("disk: pci status=0x%x cap_ptr=0x%x\n", status, cap);
  ...
  while(cap && guard++ < 64){
    ...
    printf("disk: cap@0x%x id=0x%x next=0x%x len=%d\n", cap, cap_id, next, cap_len);
    ...
    if(cap_id == PCI_CAP_ID_VNDR && cap_len >= 16){
      ...
      printf("disk:  vndr cfg=%d bar=%d off=0x%x len=0x%x bar_base=%p alloc_next=%p\n", cfg_type, bar, off, len, (void*)bar_base, (void*)pci_mmio_alloc);
...
virtio_pci_disk_init(void)
{
  ...
  uint8 cls = pci_read8(bus, dev, fun, PCI_CLASS_CODE);
  uint8 sub = pci_read8(bus, dev, fun, PCI_SUBCLASS);
  uint16 cmd = pci_read16(bus, dev, fun, 0x04);
  printf("disk: found pci dev %d:%d.%d vendor=0x%x devid=0x%x class=0x%x sub=0x%x cmd=0x%x\n", bus, dev, fun, PCI_VENDOR_QEMU, devid, cls, sub, cmd);
  if((cmd & 0x6) != 0x6){
    uint16 ncmd = cmd | 0x6;
    pci_write16(bus, dev, fun, 0x04, ncmd);
    printf("disk: enable pci cmd old=0x%x new=0x%x\n", cmd, ncmd);
  }
  ...
  printf("disk: caps common=%p notify=%p mul=%d\n", disk.common, disk.notify_base, disk.notify_off_mul);
...
disk_rw(struct buf *b, int write)
{
  ...
  while(disk.used_idx == disk.used->idx){
    spin++;
    if((spin & ((1UL<<27)-1)) == 0){
      printf("disk: wait used_idx=%d used->idx=%d avail->idx=%d qn=%d head=%d blk=%d\n",
             disk.used_idx, disk.used->idx, disk.avail->idx, disk.common->queue_size,
             idx[0], b->blockno);
    }
    if(spin > (1UL<<31)){
      printf("disk: timeout blk=%d desc_pa=%lx avail_pa=%lx used_pa=%lx\n",
             b->blockno,
             kva2pa((uint64)disk.desc),
             kva2pa((uint64)disk.avail),
             kva2pa((uint64)disk.used));
